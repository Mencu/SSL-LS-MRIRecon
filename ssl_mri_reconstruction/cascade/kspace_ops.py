...
from merlinth.layers.data_consistency import DCGD, DCPM
...
class DataConsistencyInKspace(nn.Module):
    ...
    def __init__(self, noise_lvl=None, op=None, op_adj=None):
        super(DataConsistencyInKspace, self).__init__()

        if op is None or op_adj is None:
            raise NotImplementedError('One of the Fourier functions is not implemented in data consistency!')
    ...
    def perform(self, x, k0, ktraj, smaps, dcomp):
        """
        x    - input in image domain, of shape (n, nx, ny[, nt])
        k0   - initially sampled elements in k-space
        mask - corresponding nonzero location
        """
        bs, _, nx, ny, tphases = x.shape
        # this also gets rid of batch dimension 1
        x_complex = torch.view_as_complex(torch.clone(x).permute(0,2,3,4,1).contiguous()).permute(0,3,1,2).reshape(tphases, 1, nx, ny).contiguous()       
        k = self.op(x_complex, smaps, ktraj, dcomp)
        out =  k - k0
        x_res_complex = self.op_adj.forward(out, smaps, ktraj, dcomp)
        x_res = torch.view_as_real(x_res_complex).permute(1,4,2,3,0).contiguous() # 2, nx, ny, target_phases
        return x - x_res

class DataConsistencyInKspaceUnbatched(nn.Module):
    """ Create data consistency operator
    Warning: note that FFT2 (by the default of torch.fft) is applied to the last 2 axes of the input.
    This method detects if the input tensor is 4-dim (2D data) or 5-dim (3D data)
    and applies FFT2 to the (nx, ny) axis.
    ...
    def __init__(self, noise_lvl=None, op=None, op_adj=None):
        super(DataConsistencyInKspaceUnbatched, self).__init__()
    ...
    def perform(self, x, k0, ktraj, smaps, dcomp, mask):
        """
        x    - input in image domain, of shape (n, nx, ny[, nt])
        k0   - initially sampled elements in k-space
        mask - corresponding nonzero location
        ...
        tphases, _, nx, ny = x.shape
        # this also gets rid of batch dimension 1
        # x_complex = torch.view_as_complex(torch.clone(x).permute(0,2,3,1).contiguous()).unsqueeze(1)
        x_complex = real2complex(x)

        # dcgd = DCGD(self.op, self.op_adj)
        dcgd = DCPM(self.op, self.op_adj, max_iter=5) # TODO weight_init = 10
        stacked_smap = smaps.repeat(30,1,1,1)
        res = dcgd((x_complex, k0, stacked_smap, ktraj, dcomp))

        x_res = complex2real(res)#torch.view_as_real(res.permute(0,2,3,1).contiguous().squeeze(-1)).permute(0,3,1,2).contiguous()
        return x_res
