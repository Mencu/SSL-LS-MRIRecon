...
class SSLModule(pl.LightningModule):
    def __init__(self, n_ch=2, nc=5, nd=5, op=None, op_adj=None, lr=1e-3, self_supervised=True, 
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Apply masks before model input
        ...
        out_img_real, nw_kspace_output, _, _ = self.model(img, smap, ktraj, dcomp, trn_mask, loss_mask) # _ params are rhs and penalties
        ...
        if self.self_supervised:
            ...
            out_img_real = self.crop(out_img_real)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        out_img_real, nw_kspace_output, _, _ = self.model(img, smap, ktraj, dcomp, remainder_mask, val_mask)

        out_img_complex = real2complex(out_img_real)
        out_img_complex = self.crop(out_img_complex)

        # Save image gif at every 5 epochs
        if self.current_epoch % 5 == 0:
            save_gif(out_img_complex, self.gif_root + f'pred_out_img_real_{self.current_epoch}.gif')
    ...
    def test_step(self, batch, batch_idx: int, dataloader_idx: int = 0):
        ...
        out_img, nw_kspace_output, mu_penalty, thresh = self.model(img, smap, ktraj, dcomp, remainder_mask, val_mask)

        # Center crop before comparing in image domain
        out_img_complex = self.crop(out_img)
        out_img_complex = real2complex(out_img_complex)
