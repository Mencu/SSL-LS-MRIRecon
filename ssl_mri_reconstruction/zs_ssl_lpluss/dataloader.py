...
class KspaceLoader(Dataset):
    def __init__(self, split = None, fnames = None, img_targets: dict = None, transform: Callable = None,
        ...
        # NOTE Only works for few samples (depending on your machine's cmemory capacity)
        self.samples = []
    ...
    def __getitem__(self, item):
        ...
        if self.self_supervised:
            ...
            if self.split == 'train':
                # Reshape kspace and mask to have nspokes,nFE
                kdata = kdata.reshape(kdata.shape[0], kdata.shape[1], num_spokes, -1)
                remainder_mask = remainder_mask.reshape(kdata.shape[0], kdata.shape[1], kdata.shape[2], -1)   
                
                # Masks for self-supervised
                trn_mask, loss_mask = uniform_selection(kdata.detach().cpu().numpy(), remainder_mask, rho=0.4)

                remainder_mask = remainder_mask.reshape(kdata.shape[0], kdata.shape[1], -1)
                trn_mask = trn_mask.reshape(kdata.shape[0], kdata.shape[1], -1)
                loss_mask = loss_mask.reshape(kdata.shape[0], kdata.shape[1], -1)
                kdata = kdata.reshape(kdata.shape[0], kdata.shape[1], -1)
                remainder_mask = torch.from_numpy(remainder_mask)
                trn_mask = torch.from_numpy(trn_mask)
                val_mask = torch.from_numpy(loss_mask)

                return caseid, casename, target_img, kdata, ktraj, smap, dcomp, trn_mask, loss_mask
                
            elif self.split == 'val':
                remainder_mask = torch.from_numpy(remainder_mask)
                val_mask = torch.from_numpy(val_mask)

                return caseid, casename, target_img, kdata, ktraj, smap, dcomp, remainder_mask, val_mask
...
class KspaceDataModule(pl.LightningDataModule):
    def __init__(self, batch_size: int = 1, transforms=None, root=None, img_path=None, limit_num_samples=None, 
            num_workers=32, shuffle=True, op=None, op_adj=None, img_dim=None, self_supervised=True):
        ...
        # Load all target images and store in a keyed dictionary
        self.targets = {}
        ...
        self.train_dataset = KspaceLoader('train', self.train_split, self.targets, transform=self.transforms['train'], 
                                        op=self.op, op_adj=self.op_adj, img_dim=self.img_dim, self_supervised=self.self_supervised)
    ...
    def test_dataloader(self):
        return DataLoader(self.test_dataset, batch_size=self.batch_size,
                          num_workerss=self.num_workers, pin_memory=True)
